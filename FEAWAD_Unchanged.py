...
# Changed from original FEAWAD
import csv
def get_data_dim(dataset, data_type):
    """
    Takes dataset Str and data_type (currently only datatype 0 is accepted)
    returns length of first item in dataset, minus one or two depending on dataset
    In original FEAWAD this was an argument, so I have replaced args.data_dim with get_data_dim()
    """
...
def run_devnet(args):
    ...
    for nm in names:
        ...
        if data_format == 0:
            x, labels = dataLoading(args.input_path + filename + ".csv", byte_num=get_data_dim(nm, 0))
        else:
